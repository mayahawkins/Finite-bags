...
public interface FiniteBags<I extends Comparable> extends Sequence<I>{
	...
	public FiniteBags balance();
}


public class Things_MT<I extends Comparable> implements FiniteBags<I>, Sequence {
	boolean isBlack;
	...
	Sequence seq() {returns this;}
	public boolean notEmpty() {returns false;}
...
public class Things_ST<D extends Comparable> implements FiniteBags<D>, Sequence{
	...
	public FiniteBags remove(I item, int num){
		if(item.compareTo(this.here) == 0){
			if(item >= this.amount){
				...
				return new FiniteBags(this.lefty, this.here, this.righty, (this.amount - num), this.isBlack);
			}
		}
		else if(item.compareTo(this.here < 0)) {
			return new FiniteBags(this.lefty.remove(item, num), this.here, this.righty, this.amount, this.isBlack);
		}
		else {
			return new FiniteBags(this.lefty, this.here, this.righty.remove(item, num) this.amount, this.isBlack);
		}
	}
	public FiniteBags remove(I item){
	...
	public FiniteBags inter(FiniteBags baggy){
		if(baggy.member(this.here)) {
			return new FiniteBags(this.lefty.inter(baggy), this.here.inter(baggy), this.amount, this.isBlack);
		}
		else {
